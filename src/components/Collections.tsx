...
const Collections: React.FC = () => {
    ...
    const searchImages = (searchTerm: string) => {
        ...
        const filtered = images.filter((image) =>
            image.labels.some((label) => label.description.toLowerCase().includes(searchTerm.toLowerCase())),
        );
    ...
    return (
        <div>
            <div className="flex flex-col items-center justify-center bg-[#0F172A]">
                ...
                <div className="mx-auto my-8 flex w-full max-w-md overflow-hidden rounded-md border-2 border-blue-500">
                    <input
                        type="text"
                        id="searchbar"
                        placeholder="Search description..."
                        onChange={(e) => searchImages(e.target.value)}
                        className="w-full bg-white px-4 py-3 text-sm text-black outline-none"
                    />
                    <button
                        type="button"
                        className="flex cursor-pointer items-center justify-center bg-[#007bff] px-5"
                        onClick={() =>
                            searchImages((document.getElementById("searchbar") as HTMLInputElement)?.value || "")
                        }
                    >
                        <svg
